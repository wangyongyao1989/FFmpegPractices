...
#include "includes/GetMeidaTimeStamp.h"

GetMeidaTimeStamp::GetMeidaTimeStamp() {
...
string GetMeidaTimeStamp::getMediaTimeStamp(const char *meidaPath) {

    fmt_ctx = avformat_alloc_context();
    // 打开音视频文件
    int ret = avformat_open_input(&fmt_ctx, meidaPath, nullptr, nullptr);
    if (ret < 0) {
        LOGE("Can't open file %s.\n", meidaPath);
        timeStampInfo = "Can't open file:";
    ...
    if (ret < 0) {
        ...
        timeStampInfo = "Can't find stream information.";
        return timeStampInfo;
    }
    ...
    if (video_index >= 0) {
        ...
        if (!video_codec) {
            ...
            timeStampInfo = "video_codec not found";
            return timeStampInfo;
        }
        ...
        LOGI("video timestamp_increment=%d\n", timestamp_increment);

        timeStampInfo = timeStampInfo + "视频的比特率=" + to_string(video_codecpar->bit_rate)
                        + "\n 视频的帧率=" + to_string(fps)
                        + "\n 视频时间基准 num =" + to_string(time_base.num)
                        + "\n 视频时间基准 den =" + to_string(time_base.den)
                        + "\n 视频帧的时间戳增量 =" + to_string(timestamp_increment);
    }
    // 找到音频流的索引
    ...
    if (audio_index >= 0) {
        ...
        if (!audio_codec) {
            ...
            timeStampInfo = "audio_codec not found";
            return timeStampInfo;
        }
        ...
        int timestamp_increment =
                1 * audio_codecpar->frame_size * (time_base.den / audio_codecpar->sample_rate);
        LOGI("audio timestamp_increment=%d\n", timestamp_increment);
        timeStampInfo = timeStampInfo + "音频的比特率=" + to_string(audio_codecpar->bit_rate)
                        + "\n 音频时间基准num =" + to_string(time_base.num)
                        + "\n 音频时间基准 den =" + to_string(time_base.den)
                        + "\n 音频帧的时间戳增量  =" + to_string(timestamp_increment);
    }
