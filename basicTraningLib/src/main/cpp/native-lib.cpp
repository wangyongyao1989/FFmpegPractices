...
JNIEXPORT jstring JNICALL
cpp_get_media_coder_msg(JNIEnv *env, jobject thiz, jstring videoPath) {
    ...
    if (video_index >= 0) {
        ...
        AVCodec *vide_codec = (AVCodec *) avcodec_find_decoder(video_codec_id);
        if (!vide_codec) {
            LOGE("vide_codec not find.\n");
            infoMsg = "vide_codec not find";
            return env->NewStringUTF(infoMsg.c_str());
        }
        infoMsg = infoMsg + "视频解码信息 : " + "\n video codec id= " + to_string(vide_codec->id)
                  + "\n video codec name= " + vide_codec->name;
//                  + "\n video codec long_name= " + vide_codec->long_name
//                  + "\n video codec type = " + to_string(vide_codec->type);
    }
...
//                  + "\n audio_codec long_name = " + audio_codec->long_name
...
static const JNINativeMethod methods[] = {
        ...
        {"native_get_media_coder_msg", "(Ljava/lang/String;)Ljava/lang/String;", (void *) cpp_get_media_coder_msg},
